...
| 方案 | 优点 | 缺点 | 硬件要求 | 推荐度 |
|------|------|------|----------|--------|
| **StepFun TTS** | 35种音色、情感控制、行内指令、音质优秀 | 按字符计费 | 无 | ⭐⭐⭐⭐ 首选 |
| **MiniMax TTS** | 云端克隆、速度极快（<3秒）、音质优秀 | 按字符计费 | 无 | ⭐⭐⭐ |
...
```
1. 首选 StepFun TTS
   - 检测 STEP_API_KEY 是否配置
   - 测试调用是否正常
   - 如果成功 → 使用 StepFun

2. StepFun 不可用时 → 退回 MiniMax TTS
   - 检测 MINIMAX_API_KEY 是否配置
   - 如果成功 → 使用 MiniMax

3. 都不可用时 → 退回 Edge TTS
   - 使用预设音色 zh-CN-YunyangNeural
```

---

## 方案零：StepFun TTS（首选推荐）

阶跃星辰 StepAudio 2.5 TTS，支持 35 种音色、全局语气指令和行内情感控制。

### 核心优势

1. **35 种预置音色**：覆盖男声/女声、各种风格（播音、温柔、磁性、活力等）
2. **双层情感控制**：
   - 全局指令 `instruction`：设置整体语气基调（200字以内）
   - 行内指令 `()`：逐句控制情感、停顿、呼吸（不被朗读）
3. **高质量输出**：MP3 格式，音质优秀

### 配置

1. 注册 https://platform.stepfun.com
2. 获取 API Key
3. 设置环境变量：
...
| voice_id | 名称 | 性别 | 适合场景 |
|----------|------|------|----------|
| `cixingnansheng` | 磁性男声 | 男 | 有声书、情感陪伴、教程 |
| `boyinnansheng` | 播音男声 | 男 | 专业播音腔、旁白（教程首选） |
| `zixinnansheng` | 自信男声 | 男 | 教育、营销 |
| `wenrounansheng` | 温柔男声 | 男 | 旁白、客服 |
| `elegantgentle-female` | 气质温婉 | 女 | 客服、旁白、教育 |
| `livelybreezy-female` | 活力轻快 | 女 | 营销、教育 |
| `linjiajiejie` | 邻家姐姐 | 女 | 旁白、语音助手 |
| `ruyananshi` | 儒雅男士 | 男 | 有声书、语音助手 |

完整音色列表见：https://platform.stepfun.com/docs/zh/guides/developer/tts#支持音色
...
```python
text = "（热情）欢迎来到今天的课程！（停顿）让我们开始吧。"
instruction = "语气亲切自然，像朋友聊天"
```
...
使用 `scripts/generate_audio_stepfun.py` 生成音频：
